@php $i = $item ?? $pelatih ?? null; @endphp
<div class="row g-3">
@if(empty($hideCaborSelect))<div class="col-md-6"><label class="fw-semibold">Cabor *</label><select name="cabor_id" class="form-control" required>@foreach($cabors ?? [] as $c)<option value="{{ $c->id }}" @selected(old('cabor_id', $i?->cabor_id)==$c->id)>{{ $c->name }}</option>@endforeach</select></div>@endif
...
<div class="col-md-4">@include('admin.components.competition-level-select', ['selected' => $i?->level])</div>
